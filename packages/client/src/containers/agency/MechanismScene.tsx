...
const url = `https://${window.location.hostname}:4710/api/v1/practices/mechanisms/`;
const agenciesUrl = `https://${window.location.hostname}:4710/api/v1/practices/agencies/`;
  ...
  loadMeta = async () => {
    try {
      ...
      this.setState(prevState => ({
        ...prevState,
        agencies: data.map((a: Agency) => ({ label: a.name, value: a._id }))
      }));
    } catch (e) {
      this.messages.show({
        severity: "error",
    ...
    return (
      <div>
        ...
        >
          {this.state.showForm ? (
            <MechanismForm
              mechanism={this.state.activeMechanism}
